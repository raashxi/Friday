...
> *"Good Morning, Sir. I am FRIDAY. All systems operational."*
...
FRIDAY is a **fully local, privacy first AI assistant** that runs entirely on your machine no API keys, no subscriptions. Speak a command and watch a complete intelligence pipeline activate: your voice is transcribed by Whisper, processed by a locally-running Mistral-7B, spoken back in a hand cloned FRIDAY voice via Coqui TTS, and stored in a persistent ChromaDB memory layer all while a cinematic React dashboard renders everything in real time. Automate your morning brief, manage your calendar, push Slack summaries, and organize files through a single unified interface. (Inspired BY Tony Stark)

> **Just raw, local intelligence.**
...
- **Wake-word voice interaction**: Whisper.cpp on the Neural Engine; speaks back in a hand cloned FRIDAY voice. Real two way conversation, no internet required.
- **On device LLM**: Mistral-7B-Instruct running through Ollama at **19.75 tokens/second** on Apple M2.
- **Natural FRIDAY voice**: Coqui TTS fine tuned on VCTK speaker 6097. 
- **Stark Dashboard**: Full screen holographic UI built with **React + Vite + Framer Motion**. Arc reactor animation. Rotating 3D wireframe sphere. Realtime system metrics. Particle field background. Glassmorphism panels.
...
- **Morning brief agent**: Every morning: live news via RSS + calendar events + weather → Mistral-7B synthesis → spoken brief via FRIDAY TTS + pushed to Slack. One pipeline, zero manual effoert.
...
├──   server.py               # FastAPI server /ask, /brief, /memory, /status
├──   friday_tts.py           # Coqui TTS engine FRIDAY voice synthesis
...
│   │   │   ├── VoiceVisualizer.jsx     # Realtime waveform bars
...
[FRIDAY] Coqui TTS engine ready ............. ✓  (FRIDAY voice VCTK 6097)
...
| **Text-to-Speech** | Production | Coqui TTS · FRIDAY voice clone · VCTK 6097 |
